...
export default function NewsCard({newsDetails}) {
  return (
    <Link href={newsDetails.url} target='_blank' className='bg-gray-200 w-full h-fit flex flex-row gap-4 p-4 rounded-lg'>
      <div className='flex items-center'>
        <img src={newsDetails.image} alt="news" className='size-28 rounded-lg'/>
      </div>
      <div className='flex-1 flex flex-col gap-2'>
        <div>
          <h1 className='text-xl'>{newsDetails.headline}</h1>
          <p className='text-sm'>Published {formatTimeAgo(newsDetails.datetime)} |  {formatTimePosted(newsDetails.datetime)}</p>
        </div>
        <p className='text-sm'>{newsDetails.summary}
        </p>
